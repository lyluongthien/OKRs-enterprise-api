...
export class UserStarRepository extends Repository<UserStarEntity> {
  public async getAllUserStar(): Promise<UserStarEntity[]> {
    try {
      ...
      "user"."gravatarURL", "user"."id" AS "user_id", t."name" , r."name" as rolename,
      ...
      left join roles r on r.id = "user"."roleId" 
      WHERE "cycleId" in (select "cycleId" from cycles c2) 
      GROUP BY "user"."id", "user"."fullName", t."name", r."name" 
